function results = Searchlight_RSA_Simple( varargin )
%SEARCHLIGHT_RSA_Simple Simple correlation based RSA
    p = Searchlight_RSA_Simple_Parameters();
    ...
    [ModelInstances.RandomSeed] = deal(p.Results.RandomSeed);
...
function p = Searchlight_RSA_Simple_Parameters()
    p = inputParser;
...
function SL = run_searchlight(S, radius, orientation, RandomSeed, SortByCoordsIndex)
    X = S.getData();
