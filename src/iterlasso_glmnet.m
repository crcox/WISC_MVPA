  ...
  for iSubj = 1:numel(Xtrain)
    ...
    while 1
      ...
      if isMultinomial
        ...
        [~,ix] = max(glmnetPredict(obj(iSubj),xtrain),[],2);
        yztrain_m = bsxfun(@eq, ix, 1:m);
      else
        yztest_m = glmnetPredict(obj(iSubj),xtest)>0;
        yztrain_m = glmnetPredict(obj(iSubj),xtrain)>0;
      end
      
      %% Compute iteration results
      if iscell(obj(iSubj).beta)
        wz = cell2mat(obj(iSubj).beta);
      else
        wz = obj(iSubj).beta;
      end
      nz_rows = any(wz,2);
      ...
      if iscell(obj(iSubj).beta)
        used(unused,1:m) = cell2mat(obj(iSubj).beta) ~= 0;
      else
        used(unused,1:m) = obj(iSubj).beta ~= 0;
      end
    ...
    if any(used(:))
      ...
      if isMultinomial
        ...
        W{iSubj}(any(used,2),:) = obj(iSubj).beta;
      end
    end
    I{iSubj} = iterations;
  end
