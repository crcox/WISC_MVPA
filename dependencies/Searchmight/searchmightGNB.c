                  ...
                  int nrhs, const mxArray *prhs[])
{
  ...
  /*  create hitrate matrices */
  plhs[3] = mxCreateDoubleMatrix(1,m,mxREAL);
  hitrate = mxGetPr(plhs[3]);
  hitcount = (double *) mxMalloc( m*sizeof(double) );
  for (j=0; j<m; j++) { hitcount[j] = 0; };

  /*  create false alarm matrices */
  plhs[4] = mxCreateDoubleMatrix(1,m,mxREAL);
  falsealarmrate = mxGetPr(plhs[4]);
  falsealarmcount = (double *) mxMalloc( m*sizeof(double) );
  for (j=0; j<m; j++) { falsealarmcount[j] = 0; };
  ...
  if (nClasses==2)
  {
    ntargets = 0;
    ndistractors = 0;
    for (i=0; i<n; i++) { if (labels[i]==0) {ndistractors++;ntargets++;} }
  }
  ...
  */
    ...
    */
    ...
    for (i=groupStarts[gtest]; i<=groupEnds[gtest]; i++)
    {
      ...
      for (j=0; j<m; j++)
      {
        ...
        if ( (cptr[j]==1) & (labels[i]==1) ) { hitcount[j] = hitcount[j] + 1; }
        if ( (cptr[j]==1) & (labels[i]==0) ) { falsealarmcount[j] = falsealarmcount[j] + 1; }
  ...
  for (j=0; j<m; j++)
  {
    accuracy[j] = accuracyCount[j];
    hitrate[j] = hitcount[j] / ntargets;
    falsealarmrate[j] = falsealarmcount[j] / ndistractors;
  } /* divide later, keep counts for now */
